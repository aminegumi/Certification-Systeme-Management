...
if(isset($_GET["idmsg"])){
 ...
 $idc=$_GET['idmsg'];
 $crs=course::selectcourseById('course',$connection->conn,$_GET['idmsg']);
...
if(isset($_POST['msg'])){
     $Id_Cours=$_GET['idmsg'];
     $Id_Stud=$_SESSION['idstudent'];
     $Time=date("Y-m-d H:i:s");
     $MsgText=$_POST['Message'];
     $msg=Message::SelectMsgByCourAndStudId('messages', $connection->conn,$Id_Cours,$Id_Stud);
     if(empty($MsgText)){

        $error = "filed must be filed out !";
      
      }else{
        if(empty($msg)){
            $Message = new Message($Id_Cours,$Id_Stud,$Time,$MsgText);
            $Message->insertMessage('messages', $connection->conn);
            header('Location: courses.php');
        }else{
            $error = "the Warning is already exists if you want to Write new one the previous one will disappear !";
            $Message = new Message($Id_Cours,$Id_Stud,$Time,$MsgText);
            Message::updateMessage($Message,'messages', $connection->conn,$msg['id_msg']);
            header('Location: courses.php');
        }
...
<header id="header" class="fixed-top">
  ...
  <main id="main">
    ...
    <div class="breadcrumbs" data-aos="fade-in">
        <div class="container">
            <h2>Write Message for <?php echo $crs['nom_Course'] ?> course</h2>
        </div>
        ...
        <div class="row justify-content-center">
            <div class="col-lg-6">
                ...
                <form action="" method="post" enctype="multipart/form-data">
                    <div class="mb-3">
                        <label for="message" class="form-label">Course Message</label>
                        <textarea class="form-control" id="message" name="Message" rows="4" required></textarea>
                    </div>
                    <button type="submit" name="msg" class="btn btn-success">Send Message</button>
                </form>
            </div>
        </div>
    </div>
