...
if(isset($_POST["submitMajor"])){
    ...
    // $idTeacherValue = $_POST["IdTeacher"]; hadi AI
    $MajorNameValue = $_POST["majorName"];
    $adminRespValue = $varID;
    // die($adminRespValue);

// empty($idTeacherValue) ||  hadi dart liya chawchara
    if(empty($MajorNameValue) || empty($adminRespValue)){

        $errorMesage = "all fileds must be filed out!";

    }else{

        $teacher = new filiere($MajorNameValue,$adminRespValue);

        //call the insertClient method
        $teacher->insertfiliere('filiere',$connection->conn);
        
        //give the $successMesage the value of the static $successMsg of the class
        $successMesage = filiere::$SuccMsg;
        
        //give the $errorMesage the value of the static $errorMsg of the class
        $errorMesage = filiere::$ErrMsg;
                    ...
                    <div class="bg-secondary rounded p-4 p-sm-5 my-4 mx-3">
                        <div class="d-flex align-items-center justify-content-between mb-3">
                            <a href="AdminIterface.php" class="">
                                <h3 class="text-primary"><i class="fa fa-user-edit me-2"></i>ADD MAJOR</h3>
                            </a>
                            </div>
                        ...
                        <div class="form-floating mb-3">
                            <input type="text" name="majorName" class="form-control" id="floatingText" placeholder="first name">
                            <label for="floatingText">Major Name</label>
                        </div>
                        <div class="form-floating mb-3">
                            <input type="text" name="respID" class="form-control" id="floatingText" placeholder="last name" value="<?php echo $varnom ." ".$varprenom; ?>">
                            <label for="floatingText">Respo Name</label>
                        </div>
                       <?php if(!empty($successMesage)){
                        ...
                        ?>  
                        <button type="submit" name ="submitMajor" class="btn btn-primary py-3 w-100 mb-4">Add Major</button>
                        </form>
                    </div>
                </div>
            </div>
