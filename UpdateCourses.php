...
if($_SERVER['REQUEST_METHOD']=='GET'){
    ...
    $id = $_GET['id'];
    $crs=course::selectcourseById('course',$connection->conn,$id);
    $CourseName=$crs['nom_Course'];
    $LinkToCourse=$crs['linkTOcourse'];
    $idMajor=$crs['Id_mat'];
}
if(isset($_POST["UpdateCourse"])){
    $CourseName = str_replace("'", ' ', $_POST['nameC']);
    $LinkToCourse = $_POST['LinkC'];
    $idMajor = $_POST['MajorC'];
    if(empty($CourseName) || empty($LinkToCourse) || empty($idMajor)){

        $errorMesage = "all fileds must be filed out!";

    }else{
        $Course = new course($CourseName,$LinkToCourse,$idMajor);

        //call the static updateClient method and give $client in the parameters
        course::updatecourse($Course,'course',$connection->conn, $_GET['id']);
            ...
            <div class="row h-100 align-items-center justify-content-center" style="min-height: 100vh;">
                <div class="col-12 col-sm-8 col-md-6 col-lg-5 col-xl-4">
                    <div class="bg-secondary rounded p-4 p-sm-5 my-4 mx-3">
                        ...
                        <div class="form-floating mb-3">
                            <input type="text" name="nameC" class="form-control" value="<?php echo $CourseName ?>" id="floatingText" placeholder="first name">
                            <label for="floatingText">Course Name</label>
                        </div>
                        <div class="form-floating mb-3">
                            <input type="text" name="LinkC" class="form-control" value="<?php echo $LinkToCourse ?>" id="floatingText" placeholder="first name">
                        ...
                        <div class="form-floating mb-3">
                            <select name='MajorC' class="form-select">
                                    <option value=<?php echo $idMajor ?> >Subject Destination</option>
                                     <?php
                                        $data=filiere::selectfiliereByAdminId('filiere',$connection->conn,$varID);
                                        foreach($data as $row){
                                            echo "<option disabled>$row[nom_Filiere]</option>";
                                            $mat=Matiere::selectMatiereByfiliereId('matiere',$connection->conn,$row['id_Filiere']);
                                            foreach($mat as $m){
                                                echo "<option value='$m[id_Mat]' >$m[nom_Matiere]</option>";
                                            }
                                        }
                                    ?>
                            </select>
                       ...
                       <?php if(!empty($successMesage)){
                        ...
                        ?>  
                        <button type="submit" name ="UpdateCourse" class="btn btn-primary py-3 w-100 mb-4">Update Course</button>
                        </form>
                    </div>
                </div>
            </div>
