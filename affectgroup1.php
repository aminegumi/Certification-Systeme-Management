...
if(isset($_POST["assign"])){
    ...
    if(empty($idMajor) || empty($groupe) || empty($idTeacher) || empty($idSubject)){

        $errorMesage = "all fileds must be filed out!";

    }else{
        if($groupe=='all'){
            $std=student::selectStudentByfiliereId('student',$connection->conn,$idMajor);
            foreach($std as $s){
                $test=Student_Teacher::selectStudentByTeaId('studentteacher',$connection->conn,$idTeacher,$s['id_student']);
                if(empty($test)){
                    $studentTeacher = new Student_Teacher($idTeacher,$s['id_student'],$idSubject);
            ...
            foreach($data1 as $row){
                $studentTeacher = new Student_Teacher($idTeacher,$row['id_student'],$idSubject);
                $studentTeacher->insertStudentTeacher('studentteacher',$connection->conn);
            }
            $successMesage = Student_Teacher::$successMsg;
            $errorMesage = Student_Teacher::$errorMsg; 
                ...
                <div class="col-12 col-sm-8 col-md-6 col-lg-5 col-xl-4">
                    <div class="bg-secondary rounded p-4 p-sm-5 my-4 mx-3">
                        <div class="d-flex align-items-center justify-content-between mb-3">
                            <h3 class="text-primary"><i class="fa fa-user-edit me-2"></i>Create Group</h3>    
                        </div>
                        ...
                        <div class="form-floating mb-3">
                            ...
                            <div class="mb-3">
                            <select name='groupes' class="form-select">
                                <option>Select a Group</option>
                                <?php
                                foreach($data as $row){
                                    echo "<option disabled>$row[nom_Filiere]</option>";
                                    echo "<option value='all'>ALL</option>";
                                    $group = Student::selectStudentDistinctGroupe('student', $connection->conn, $row['id_Filiere']);
                                    foreach($group as $r) {
                                        echo "<option value='$r[groupe]'>$r[groupe]</option>";
                                    }
                                }
                                ?>
                            </select>
                       ...
                       <div class="mb-3">
                        <select name='teachers' class="form-select">
                            <option>teacher</option>
                                     <?php
                                        $data=Teacher::SelectALLTeachers('teacher',$connection->conn);
                                        foreach($data as $row){
                       ...
                       <?php if(!empty($successMesage)){
                        ...
                        ?>  
                        <button type="submit" name ="assign" class="btn btn-primary py-3 w-100 mb-4">Create Groupe</button>
                        </form>
                    </div>
                </div>
            </div>
