...
if(isset($_POST["submitGroups"])){
    ...
    if(!empty($_FILES['file']['name']) && in_array($_FILES['file']['type'], $excelMimes) ){
        ...
        $keyscompleted = array_keys($tabprog);
        $totalKeyscompleted = count($keyscompleted);
        foreach ($tabprog as $gri => $prog) {

            $totalgp = student::selectCountStudentGrp('student', $connection->conn, $gri);
            $completed = $prog[0];
            $inprogress= $prog[1];
            $unenrolled = $totalgp - $completed - $unenrolled;
            $progress= new Progress($idcc,$completed,$unenrolled,$inprogress,$gri);
            $progress->insertcourse('progress',$connection->conn);
            $unenrolled=0;
                    ...
                    <div class="bg-secondary rounded p-4 p-sm-5 my-4 mx-3">
                        <div class="d-flex align-items-center justify-content-between mb-3">
                            <a href="AdminIterface.php" class="">
                                <h3 class="text-primary"><i class="fa fa-user-edit me-2"></i>ADD Teachers</h3>
                            </a>
                            </div>
